...
public class SalesmapController {
    ...
    public ApiResponse<SalesmapRegisterResponse> register(@Valid @RequestBody SalesmapRegisterRequest request) {
        return ApiResponse.success("Salesmap 등록 요청이 완료되었습니다.", salesmapService.register(request));
    }

    @GetMapping("/analysis/{analysisId}")
    public ApiResponse<List<SalesmapRegisterResponse>> getRecordsByAnalysis(@PathVariable Long analysisId) {
        return ApiResponse.success(salesmapService.getRecordsByAnalysis(analysisId));
    }
}
