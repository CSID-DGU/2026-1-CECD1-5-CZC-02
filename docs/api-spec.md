...
설명:
로그인한 사용자 기준으로 `sourceId`를 조회한 뒤 mock 분석 결과를 DB에 저장합니다. 분석 생성 시 Source 상태는 `ANALYZED`로 변경됩니다. 다른 사용자의 Source로 분석을 요청하면 `403`을 반환합니다.
...
403 Forbidden - Source Owner Mismatch:
...
설명:
로그인한 사용자 기준으로 DB에 저장된 AI 분석 결과를 조회합니다. Analysis는 연결된 Source의 사용자 기준으로 소유권을 검증합니다.
...
설명:
로그인한 사용자 기준으로 원본 데이터 ID에 연결된 AI 분석 결과 목록을 조회합니다. 다른 사용자의 Source ID로 조회하면 `403`을 반환합니다.
...
설명:
로그인한 사용자 기준으로 `analysisId`를 조회한 뒤 Salesmap 등록 이력을 `salesmap_records` 테이블에 저장합니다. 실제 Salesmap 외부 API는 아직 호출하지 않고, mock `externalRecordId`, `requestPayload`, `responsePayload`를 저장합니다. 등록 성공 시 Analysis 상태는 `APPROVED`로 변경됩니다. 다른 사용자의 Analysis로 Salesmap 등록을 요청하면 `403`을 반환합니다.
...
설명:
로그인한 사용자 기준으로 분석 결과 ID에 연결된 Salesmap 등록 이력 목록을 조회합니다. SalesmapRecord는 Analysis -> Source -> User 기준으로 소유권을 검증합니다.
